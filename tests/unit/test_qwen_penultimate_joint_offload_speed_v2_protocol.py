...
CONFIG = ROOT / "configs/benchmark/qwen_penultimate_joint_offload_speed_v2.yaml"
SAMPLES = ROOT / "configs/benchmark/qwen_penultimate_joint_offload_speed_v2_samples.json"
...
def test_v2_changes_only_the_batching_sensitive_internal_identity_gate() -> None:
    config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
    ...
    assert gate["require_sequential_bridge_sampled_token_exact"]
    assert gate["require_batching_difference_metrics_recorded"]
    assert gate["sequential_bridge_natural_route_ids_exact"].startswith("report_only")
    assert gate["sequential_bridge_executed_route_ids_exact"].startswith("report_only")
    assert gate["sequential_next_b32_exact"].startswith("report_only")
