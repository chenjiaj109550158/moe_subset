# Qwen penultimate-token joint offload speed pilot v1
...
bridge-plus-pseudo reference on disposable state. The bridge sampled token,
bridge natural and executed route IDs, and next layer-local B=32 subsets must
match exactly. The production cache must advance by one position, pseudo KV
